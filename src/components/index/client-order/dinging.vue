...
export default {
  ...
  methods: {
    lischeng () {
      ...
      var datas = [];
      let _this = this;
      const url = '/api/brief/getBriefListByStatus?status=0&start=0&length=' + _this.pageSize;
    ...
    console (currentPage, pageSize) {
      ...
      var datas = [];
      let _this = this;
      const url = '/api/brief/getBriefListByStatus?status=0&start=' + currentPage + '&length=' + pageSize;
      this.$api.get(url).then(function (res) {
        if (res.status === 200) {
          for (let i = 0; i < res.data.data.length; i++) {
            var obj = {};
    ...
    handleSizeChange (val) {
      this.pageSize = val;
      console.log(this.currentPage - 1, val);
