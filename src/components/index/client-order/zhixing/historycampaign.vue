...
export default {
  ...
  methods: {
      ...
      this.$api.get(url).then(function (res) {
        if (res.status === 200 && res.data.recordsFiltered > 0) {
          ...
          for (let i = 0; i < res.data.data.length; i++) {
            ...
            _this.$store.state.yimei.totalcont3 = res.data.recordsFiltered;
          };
        }
      });
    },
    ...
    childrenEventIsShow (val) {
      this.show = [];
      let _this = this;
      const url = '/api/campaign/getAllCampaign?status=2&start=0&length=10&industryId=2';
      this.$api.get(url).then(function (res) {
        if (res.status === 200 && res.data.recordsFiltered > 0) {
          console.log(res.data.data);
          for (let i = 0; i < res.data.data.length; i++) {
    ...
    handleClick (tab, event) {
      ...
      var datas = [];
      let _this = this;
      const url = '/api/campaign/getAllCampaign?status=2industryId=' + this.industryId + '&start=0&length=' + _this.pageSize;
      this.$api.get(url).then(function (res) {
        if (res.status === 200) {
          console.log(res);
          for (let i = 0; i < res.data.data.length; i++) {
    ...
    console (currentPage, pageSize) {
      ...
      var datas = [];
      let _this = this;
      const url = '/api/campaign/getAllCampaign?status=2&industryId=1&start=' + currentPage + '&length=' + _this.pageSize;
      this.$api.get(url).then(function (res) {
        if (res.status === 200) {
          console.log(res);
          for (let i = 0; i < res.data.data.length; i++) {
