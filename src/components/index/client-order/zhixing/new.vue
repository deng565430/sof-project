...
export default {
  ...
  methods: {
    loadData (pageNum, pageSize) {
      ...
      this.$ajax({
        method: 'get',
        url: '/api/CampaignUpload/getAllProject?status=0&start=0&length=' + _this.pageSize
      }).then(function (res) {
        if (res.status === 200) {
    ...
    console () {
      ...
      this.$ajax({
        method: 'get',
        url: '/api/CampaignUpload/getAllProject?&start=0&length=' + _this.pageSize
      }).then(function (res) {
        console.log(res);
        if (res.status === 200) {
    ...
    handleEdit (index, row) {
      window.location.href = 'http://localhost:8080/#/client/dingchange?id= ' + row.id;
    },
    search () {
      var _this = this;
      var data = [];
