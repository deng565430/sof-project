...
export default {
  ...
  created () {
    ...
    this.lists(2, '/api/campaign/getAllCampaign?status=0&industryId=2&start=0&length=10');
  },
  methods: {
    lists (id, url) {
      this.show = [];
    ...
    handleClick (tab, event) {
      ...
      this.$ajax({
        method: 'get',
        url: '/api/campaign/getAllCampaign?status=0industryId=' + this.industryId + '&start=0&length=' + _this.pageSize
      }).then(function (res) {
        if (res.status === 200) {
          console.log(res);
          for (let i = 0; i < res.data.data.length; i++) {
    ...
    loadData (pageNum, pageSize) {
      ...
      this.$ajax({
        method: 'get',
        url: '/api/campaign/getAllCampaign?status=0start=0&length=' + _this.pageSize
      }).then(function (res) {
        if (res.status === 200) {
